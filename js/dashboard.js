...
async function crearProducto() {
  ...
  if (productoEditandoId) {
    const { error } = await supabaseClient
      .from("productos")
      .update({ nombre, precio, categoria, imagen_url, descripcion, tipo_venta: tipoVenta, subcategoria })
    ...
    const { error } = await supabaseClient
      .from("productos")
      .insert([{ nombre, precio, categoria, imagen_url, descripcion, tipo_venta: tipoVenta, subcategoria }]);
...
function editarProducto(id) {
  ...
  // Mostrar/ocultar subcategoría y seleccionar valor
  toggleSubcategoria();
...
function renderRepartidores(lista) {
  ...
  // Filtrar solo pedidos en efectivo Y entregados
